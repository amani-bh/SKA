...
const onDragEndItemBackend = async (board, setBoard, result) => {
    ...
    const {data} = await axios.put(`http://localhost:8004/task-api/update_item_order/${item.id}`, {
       'title' : item.title,
       'order' : newOrder,
...
const onDragEndListBackend = async (board,setBoard,result) => {
    ...
    const {data} = await axios.put(`http://localhost:8004/task-api/update_list_order/${list.id}`, {
       'title' : list.title,
       'order' : newOrder
    })
